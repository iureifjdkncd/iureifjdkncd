...
| **Database** | `MongoDB` 및 `PostgreSQL`에서 조건 필터링, 정렬, `limit` 쿼리 수행,<br>조회 및 가공 경험                   | NoSQL(MongoDB)와 RDB(PostgreSQL) Python 연동 경험,<br>설계 및 운영 경험은 없음 | 중    |
| **Git**      | GitLab에 업로드 및 commit/pull/push,<br>`master` 브랜치 관리 경험                                            | Git 기본 명령 및 협업 경험 보유,<br>MR 및 전략적 브랜치 운영 경험 부족           | 중\~하    |
