...
| **FastAPI**  | 학습된 AI모델 `REST API`로 배포하여 시스템 구현 지원,<br>`.bat` 스크립트를 통한 `Uvicorn` 서버 자동 실행 환경 구성                                        | API 기반 시스템 구축 경험 보유,<br>개발팀 주도의 서비스 통합·배포 과정에서 협업 중심 참여             | 중    |
| **Docker**   | `Docker`를 활용한 환경 구성 및 스크립트 작성                              | 운영/배포 환경은 개발팀 주도,<br>환경 테스트 및 협업 중심으로 참여                 | 중    |
